...
Coherence는 Apple Silicon (M1/M2/M3/M4) 기반 Mac에서 USB4/Thunderbolt를 통해 연결된
...
다음 접두사는 모두 동일한 의미를 가지며, `coherence-cli` 대신 사용할 수 있습니다:
...
# 팬 속도 수동 설정 (또는 auto)
coherence-cli config fan 80
coherence-cli config fan auto

# 성능 상태 변경
coherence-cli config pstate P0

# 시스템 진단
coherence-cli diag

# 오류 로그 확인
coherence-cli log -n 50 --level ERROR

# 벤치마크 실행
coherence-cli benchmark

# JSON 출력 (스크립트 연동용)
coherence-cli info --json
coherence-cli status --json
...
# 각 명령의 상세 도움말
coherence-cli help connect
coherence-cli help config
...
```bash
# CLI에서 상세 모드로 실행
coherence-cli -v connect

# 로그 파일 실시간 확인
tail -f /tmp/coherence_logs/*.log

# 오류 로그만 확인
coherence-cli log --level ERROR
```
...
├── tests/                # 테스트
├── docs/                 # 문서
│   └── README_OtherLanguage/  # 다국어 README
