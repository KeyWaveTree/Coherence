...
Coherence是一个软件栈，使Apple Silicon (M1/M2/M3/M4) Mac能够通过USB4/Thunderbolt连接的
...
### 1. GPU驱动CLI (`coherence-cli`)

类似nvidia-smi的GPU控制和监控命令行工具。

**CLI命令前缀:**

以下前缀具有相同含义，可以代替`coherence-cli`使用:
...
```bash
coherence-cli connect        # 连接eGPU
coherence-cli info            # 显示GPU信息
coherence-cli status          # 显示状态
coherence-cli diag            # 运行诊断
coherence-cli disconnect      # 安全断开
```
...
- **署名与致谢**: 如果您在企业或个人项目中使用了本代码来制作产品或服务，请在产品介绍页面、
  博客、演示文稿或源代码注释中简要提及原作者（Coherence Contributors）。
