...
Coherence is a software stack that enables CUDA computing on external NVIDIA GPUs
...
├─────────────────────────────────────────────────────────────────────────┤
│   Python API (cudabridge.py)  │  Coherence Userspace Library            │
│   numpy-compatible GPU ops    │  (libcudabridge.dylib / API Layer)       │
├─────────────────────────────────────────────────────────────────────────┤
│   CLI Tool (coherence-cli)   │  CUDA Runtime Bridge                     │
...
### 1. GPU Driver CLI (`coherence-cli`)

A command-line tool for GPU control and monitoring, similar to nvidia-smi.

**CLI Command Prefixes:**

The following prefixes are equivalent and can be used instead of `coherence-cli`:

| Prefix | Example |
|--------|---------|
| `coherence-cli` | `coherence-cli info` |
| `cophase` | `cophase info` |
| `co-` | `co- info` |
| `phase` | `phase info` |
| `cph` | `cph info` |
...
# Connect eGPU
coherence-cli connect

# Real-time monitoring (500ms interval)
coherence-cli monitor -i 500

# Show current settings
coherence-cli config show

# Change clock speeds
coherence-cli config clock 2100 1200

# Set power limit
coherence-cli config power 350

# JSON output (for scripting)
coherence-cli info --json

# Safe disconnect
coherence-cli disconnect

# Force disconnect (emergency)
coherence-cli disconnect --force
```
...
**Log file location:** `/tmp/coherence_logs/`
...
```bash
git clone https://github.com/KeyWaveTree/Coherence.git
cd Coherence
...
Build outputs:
- `coherence-cli` - GPU driver CLI tool
...
```bash
coherence-cli connect        # Connect eGPU
coherence-cli info            # Show GPU info
coherence-cli status          # Show status
coherence-cli diag            # Run diagnostics
coherence-cli disconnect      # Safe disconnect
```
...
```bash
coherence-cli -v connect                    # Verbose mode
tail -f /tmp/coherence_logs/*.log           # Live log monitoring
coherence-cli log --level ERROR             # Error logs only
```
...
- **Credit and Thanks**: If you use this code in a product or service, please mention the
  original authors (Coherence Contributors) in your product page, blog, or source code comments.
