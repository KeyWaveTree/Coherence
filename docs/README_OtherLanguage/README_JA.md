...
Coherenceは、Apple Silicon (M1/M2/M3/M4) ベースのMacで、USB4/Thunderboltを通じて接続された
...
以下のプレフィックスはすべて同じ意味を持ち、`coherence-cli`の代わりに使用できます:
...
```bash
coherence-cli connect        # eGPU接続
coherence-cli info            # GPU情報表示
coherence-cli status          # ステータス表示
coherence-cli diag            # 診断実行
coherence-cli disconnect      # 安全切断
```
...
- **出典表記と謝辞**: このコードを活用して製品やサービスを作られた場合、製品紹介ページ、ブログ、
  発表資料、またはソースコードのコメントに原作者（Coherence Contributors）への謝辞を
  記載していただけると幸いです。
